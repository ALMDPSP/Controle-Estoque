...
def listar_cadastro_itens(tipo=None):
    """Lista todos os itens do cadastro mestre.

    O parâmetro tipo é mantido por compatibilidade com versões anteriores,
    mas não filtra mais os resultados: todo item cadastrado fica disponível
    para Estoque e Imobilizados.
    """
    conn = get_conn()
    cur = get_cursor(conn)
